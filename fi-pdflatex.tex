...
Researchers in this field focused mainly on decision-making systems to support work of the jury; however, our research focuses more on the information retrieval and knowledge discovery in the legal domain, also known as \textit{Electronic Discovery}, which brings a support for lawyers.
Therefore, the second section looks more closely at this subfield by introducing state-of-the-art legal retrieval systems and underlines common characteristics with our research.
...
\begin{quote}
``Non  of  the  ontologies  seems  to  have  adequate  provisions  to  specify  legal procedures.'' --- \citeauthor{Visser1998ontologies} \cite{Visser1998ontologies}
\end{quote}
...
Despite all the effort, the lawyers were rather skeptical about AI helping in the legal domain, as can be seen in the article \citetitle{oskamp2002ainotsomuch} by \citeauthor{oskamp2002ainotsomuch} (\citeyear{oskamp2002ainotsomuch}) \cite{oskamp2002ainotsomuch}.
...
The mentioned technique is usually applied in the topic modeling of documents and is briefly explained in the section \ref{topic_modeling} as it is one of our working attributes.
...
The main goal of our research is the relevance prediction.
However, a categorization of documents is also part of our research, because the preparation for the relevance prediction is labeling documents as case-related or law-related (\ref{sec:law-case-classifier}).
...
For the purpose of our categorization, we applied the decision tree for its satisfactory performance, simplicity, and transparentness.
Moreover, the ensemble of decision trees -- Random Forest -- helped us to boost the performance.

The first enhancement of IR -- Intelligent relevance feedback -- best explains the workflow of our system.
...
Our system provides a possibility of immediate feedback through relevance settings of individual documents.
Relevances within the scope of the particular legal case are updated after each feedback.

To compare the existing systems with ours, it is worth noting that most of them implement a slightly different use case.
Their goal is to pick relevant documents out of a massive storage of legal texts, in which documents might relate to multiple cases, whereas our goal is to pick relevant documents out of a relatively small collection that is uploaded to the system by the lawyer.
We are not allowed to share documents among cases and/or look for information elsewhere than what is selected by the lawyer.
...
Most of the similar researches focus either on the case retrieval or document retrieval from a global storage, hence our use case seems to be too unusual to be included in any recent research.

% Papers predicting outcomes of new cases based on previous
% 1. Automatically classifying case texts and predicting outcomes

\section{Measuring relevance in the legal domain}
...
In the legal domain, both high precision and recall are required, sometimes the recall is considered the dominant measure as missing a relevant information could be fatal \cite{Hogan2010sensemaking}.
